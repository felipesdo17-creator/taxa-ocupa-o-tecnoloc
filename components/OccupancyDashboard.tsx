    ...
    <div className="bg-white p-6 md:p-10 rounded-[2.5rem] border border-gray-100 shadow-sm mb-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Distribuição de Status</p>
        </div>
      ...
      <div className="h-[300px] md:h-[400px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 60 }}>
            ...
            <Tooltip 
              cursor={{fill: '#f8fafc'}}
              contentStyle={{ borderRadius: '20px', border: 'none', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', fontSize: '11px' }}
            />
  ...
  return (
    <div className="space-y-6 md:space-y-10 animate-in fade-in duration-700 pb-12">
      ...
      <div className="flex overflow-x-auto no-scrollbar items-center gap-3 bg-white p-3 md:p-4 rounded-3xl border border-gray-100 shadow-sm w-full md:w-fit">
        ...
        <span className="text-[10px] font-black text-accent uppercase tracking-widest shrink-0">Filtrar:</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <KpiCard title="Ocupação Média" value={`${stats.rate.toFixed(1)}%`} icon={Activity} color="green" />
        <KpiCard title="Locados" value={stats.rented} subtitle={`Frota: ${stats.total}`} icon={CheckCircle} color="orange" />
      ...
      <div className="bg-white p-6 md:p-10 rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
        <h3 className="text-xl font-black text-accent mb-8 uppercase tracking-tighter">Performance por Modelo</h3>
                ...
                <table className="w-full text-left min-w-[600px]">
                  <thead>
                    <tr className="border-b border-gray-50">
                      <th className="py-4 text-[9px] font-black text-gray-400 uppercase tracking-widest">Modelo</th>
                      <th className="py-4 text-[9px] font-black text-gray-400 uppercase tracking-widest text-center">Lib.</th>
                      <th className="py-4 text-[9px] font-black text-gray-400 uppercase tracking-widest text-center">Manut.</th>
                      <th className="py-4 text-[9px] font-black text-gray-400 uppercase tracking-widest text-center">Locado</th>
                      <th className="py-4 text-[9px] font-black text-gray-400 uppercase tracking-widest text-right">TO %</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {group.items.map((item) => (
                      <tr key={item.name} className="hover:bg-gray-50/50 transition-colors group">
                        <td className="py-4 font-bold text-accent text-xs group-hover:text-primary transition-colors">{item.name}</td>
                        <td className="py-4 text-[11px] text-blue-600 font-bold text-center bg-blue-50/20">{item.liberado}</td>
                        <td className="py-4 text-[11px] text-orange-600 font-bold text-center">{item.manutencao}</td>
                        <td className="py-4 text-[11px] text-accent font-black text-center bg-gray-50/50">{item.locado}</td>
                        <td className="py-4 text-right">
                          <div className="flex items-center justify-end gap-3">
                            ...
                            <span className="text-[11px] font-black text-primary min-w-[35px]">{item.to.toFixed(0)}%</span>
                          </div>
                        </td>
                      </tr>
                    ))}
