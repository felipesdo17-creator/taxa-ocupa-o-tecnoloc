...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      <div
        className={`fixed inset-0 z-40 bg-accent/40 backdrop-blur-sm transition-opacity duration-300 md:hidden ${
      ...
      <aside
        className={`fixed left-0 top-0 z-50 flex h-screen flex-col border-r border-white/5 bg-accent text-white transition-all duration-500 ease-in-out md:z-40 ${
      ...
      >
        <div className="flex h-24 items-center justify-between border-b border-white/5 px-4 md:px-6">
          ...
          <button
            onClick={() => setIsMobileOpen(false)}
            className="rounded-2xl p-2 text-gray-400 transition-colors hover:bg-white/5 hover:text-white md:hidden"
          >
            <X size={20} />
          </button>
        </div>
          ...
          {filteredItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isCollapsed && isActive && !isMobileOpen && (
        ...
        <div className="space-y-2 border-t border-white/5 p-3 md:p-4">
          <button
            ...
            className="hidden md:flex w-full items-center gap-4 rounded-xl px-3 md:px-4 py-3 text-gray-400 transition-all hover:bg-white/5 hover:text-white"
          >
