...
import { Search, Filter, Download, ChevronLeft, ChevronRight, Eye, MapPin, Box } from 'lucide-react';
...
const EquipmentTable: React.FC<EquipmentTableProps> = ({ data }) => {
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      const matchesSearch = item.patrimonio.toLowerCase().includes(searchTerm.toLowerCase()) || 
  ...
  const getStatusStyle = (status: EquipmentStatus) => {
    switch (status) {
      case EquipmentStatus.LOCADO: return 'bg-green-100 text-green-700 border-green-200';
      case EquipmentStatus.EM_MANUTENCAO: return 'bg-orange-100 text-orange-700 border-orange-200';
      case EquipmentStatus.LIBERADO: return 'bg-blue-100 text-blue-700 border-blue-200';
      case EquipmentStatus.VENDIDO: return 'bg-gray-100 text-gray-700 border-gray-200';
      case EquipmentStatus.DEVOLUCAO: return 'bg-purple-100 text-purple-700 border-purple-200';
      default: return 'bg-slate-100 text-slate-700 border-slate-200';
    }
  };

  const exportToCSV = () => {
    const headers = ['Patrimonio', 'Nome', 'Tipo', 'Modelo', 'Estado', 'Status', 'Série', 'Ano Fabric.', 'Pos. Contador', 'Cont. Acum.'];
    ...
    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-gray-50/50 border-b border-gray-50">
            <tr>
              <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Ativo / Modelo</th>
              <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Ano / Estado</th>
              <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Status</th>
              <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-center">Contadores</th>
              ...
              <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-primary font-black uppercase tracking-tight">{item.modelo}</span>
                    <span className="text-[10px] text-gray-400 font-medium truncate max-w-[180px]">{item.nome_bem}</span>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                ...
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded-lg text-[9px] font-bold uppercase border ${getStatusStyle(item.status)}`}>
                    {item.status}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <div className="flex justify-center gap-6">
                    <div className="text-center">
                       <p className="text-xs font-bold text-accent">{item.pos_contador.toLocaleString()}</p>
                       <p className="text-[9px] text-gray-400 font-medium">Pos. Cont</p>
                    </div>
                    <div className="text-center">
                       <p className="text-xs font-bold text-accent">{item.contador_acumulado.toLocaleString()}</p>
      ...
      {filteredData.length === 0 && (
        <div className="p-12 text-center">
          <p className="text-gray-400 font-medium">Nenhum equipamento encontrado para os filtros selecionados.</p>
        </div>
      )}
